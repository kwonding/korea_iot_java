...
// === 롬복(Lombok) 라이브러리 === //
// : 자바에서 반복적으로 작성해야하는 보일러 플레이트(boilerplate) 코드를 줄여주는 라이브러리
...
// 3) @RequiredArgsConstructor: final 또는 @NonNull 어노테이션이 붙은 필드를 인자로 가지는 생성자

// 4. Data 어노테이션
// @Getter, @Setter, @ToString, @RequiredConstructor, @EqualsAndHashCode 어노테이션을
//      한 번에 적용하는 어노테이션
...
class LombokClass {
    ...
    // @getter / @Setter 메서드는 각 필드마다 모두 생성
...
public class G_Lombok {
    public static void main(String[] args) {
        ...
        LombokClass lombokClass3 = new LombokClass.LombokClassBuilder()
                // @Builder에서 필수 체크 값 X
                // : Lombok이 내부에서 빌어 전용 생성자를 생성 초기화를 우회 (예외)
                .title("자바자바자바")
