...
// == 다형성(Polymorphism) == //
// : 많은 형태를 가질 수 있는 특성
// - 하나의 객체가 여러 타입의 인스턴스(객체)로 표현 될 수 있는 특성
...
class Bird {
    void fly() {
        System.out.println("새가 날갯짓을 합니다.");
    }
}
...
//        Bird bird = new Bird();
//        Vehicle vBird = (Vehicle) bird; // 이런건 없음

        // cf) instanceof 연산자
        // : 참조변수값 instanceof 클래스타입
        // > 해당 객체가 특정 클래스의 인스턴스인지 또는 그 클래스를 상속받은 자식 클래스의 인스턴스 인지 확인
        // > boolean 타입으로 반환
        // 업캐스팅 된 객체를 다운캐스팅할 경우 instanceof 클래스타입의 객체가 맞는지 확인 후 캐스팅

        if (subway instanceof Vehicle) {
            // subway 객체가 Vehicle 인스턴스 || Vehicle을 상속받은 인스턴스일 경우
