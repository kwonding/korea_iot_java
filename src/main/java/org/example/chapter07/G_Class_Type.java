...
public class G_Class_Type {
    ...
    public static void main(String[] args) {
        ...
        GParent childObject = new GChild();
...
//         childObject.childmethod();
        ...
        GChild childObject2 = new GChild();
        childObject2.displayInfo(); // 자식 클래스입니다. (재정의)
        childObject2.childMethod(); // 자식 클래스만이 가지는 메서드입니다.

        // 부모클래스 객체변수명 = 자식객체변수;
        GParent parentObject = childObject2; // 업캐스팅
        parentObject.displayInfo(); // 자식 클래스입니다.
        // parentObject.childMethod(); //'GParent'의 메서드 'childMethod'을(를) 해결할 수 없습니다

        // 2. 강제 타입 변환(다운캐스팅, DownCasting)
        // : '부모 클래스 타입으로 변환된 객체를' 다시! 자식 클래스 타입으로 변환하는 것 (업캐스팅 전제)
        // - 자식 클래스에만 정의된 멤버에 다시 접근 가능

        // 자식 타입 객체변수명 = (자식타입) 부모객체;
        GChild gchild = (GChild) parentObject;
        gchild.displayInfo(); // 자식 클래스입니다.
        gchild.childMethod(); // 자식 클래스만이 가지는 메서드입니다.
