...
* 3. 외부 클래스 멤버 접근 - 내부 클래스는 외부 클래스의 필드와 메서드에 직접적인 접근 가능
*
* === 내부 클래스 종류 ===
...
*   >> 외부 클래스의 정적 멤버에만 접근 가능 (인스턴스 불가)
*
* 3. 메서드 내부 클래스 (지역 클래스)
*   : "메서드 내에서 정의된 내부 클래스"
*   >> 해당 메서드 내에서만 사용 가능
*   >> 메서드의 지역 변수에만 접근 가능
*   cf) 로컬 변수(외부 클래스의 멤버 변수)가 final인 경우에만 접근 가능
*
* 4. 익명 (내부) 클래스
*   : 이름이 없는 내부 클래스
...
class OuterClass {
    ...
    void outerMethod () {
        ...
        //      >> 재할당이 이루어지는 경우 final이 사라짐 (final로 취급 X)
        String localVar = "메서드 로컬 변수";
        // localVar = "메서드 로컬 변수 값 변경"(재할당);
...
public class A_Inner {
    public static void main(String[] args) {
        ...
        // >> 클래스 정의가 내부에서 재사용 될 필요가 없거나, 단 한 번의 사용 목적에 유용
