...
@AllArgsConstructor // @Setter 역할 대신
@Getter
@ToString
class Employee {
...
public class K_Practice {
    public static void main(String[] args) {
        ...
        List<Employee> salaries = employees.stream()
                .filter(salary -> salary.getSalary() >= 400)
        ...
        List<String> employeesNames = employees.stream()
                .filter(depart -> depart.getDepartment().contains("IT"))
...
//                .map(e -> e.getName()))
        ...
        List<Employee> employeeNamesByCho = employees.stream()
                .filter(d -> d.getDepartment().equals("HR"))
                .filter(n -> n.getName().contains("조"))
                .collect(Collectors.toList()); // .toList()

        System.out.println("HR부서 직원 중 이름에 '조'가 포함된 직원");
        employeeNamesByCho.forEach(System.out::println);
