...
* === 스트림(Stream, 흐름) API ===
* : 자바 8부터 도입된 기능
*   , 컬렉션 또는 배열에 저장된 데이터를 선언형(직관적, 간결함) 방식으로 처리할 수 있게 지원하는 도구
* - 데이터 필터링, 매핑(변환), 집계 등 다양한 작업 수행
* - 람다식과 결합하여 가독성 높은 코드 작성 가능
*
...
//        streamNames.filter(name -> name + "님"); // 최종 연산자 없어서 오류남
        ...
        //      cf) 문자열.startsWith("문자열"): 해당 문자열로 시작 여부를 반환 (boolean 값으로)
        //      cf) 문자열.contains("문자열"): 해당 문자열 포함 여부를 반환 (boolean 값으로)
